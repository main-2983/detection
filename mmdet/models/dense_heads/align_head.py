...
@HEADS.register_module()
class AlignHead_v2(FCOSHead):
    """ FCOS Head with simOTA Assigner but use IoU branch instead of Centerness """
    def __init__(self,
                 num_classes,
                 in_channels,
                 loss_lqe=dict(
                 ...
                 **kwargs):
        super(AlignHead_v2, self).__init__(
            num_classes,
            in_channels,
            **kwargs
        )
    ...
    def _get_target_single(self, cls_preds, lqe_preds, priors, decoded_bboxes,
        ...
        #                        self.num_classes)
        bbox_target = sampling_result.pos_gt_bboxes
        lqe_target = torch.zeros_like(lqe_preds)
        lqe_target[pos_inds] = assign_result.max_overlaps[pos_inds]
        foreground_mask = torch.zeros_like(lqe_preds).to(torch.bool)
        foreground_mask[pos_inds] = 1
        return (foreground_mask, cls_target, lqe_target, bbox_target, num_pos_per_img)
